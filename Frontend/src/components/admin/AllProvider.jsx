...
import { FiSearch, FiUserPlus, FiMail, FiUsers, FiEdit3, FiEye, FiSettings } from 'react-icons/fi'
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-black text-[#1a1f2e] tracking-tight">Service Providers</h1>
          <p className="text-gray-500 mt-1 font-medium">Monitoring and managing professional service partners.</p>
        </div>
        <Link
          to="/admin/addUser"
        ...
        >
          <FiUserPlus size={18} /> Register Provider
        </Link>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-[3rem] p-24 text-center border border-dashed border-gray-200">
          ...
          <h3 className="text-2xl font-black text-[#1a1f2e]">No Providers Found</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-8">
          {filtered.map((user) => (
            ...
            >
              ...
              <div className="absolute top-0 left-0 w-20 h-20 bg-blue-50/50 rounded-br-[4rem] group-hover:bg-[#1a1f2e]/10 transition-colors"></div>

              {/* Avatar */}
              <div className="relative mb-6">
                <img 
                    src={user.profilePicture || `https://ui-avatars.com/api/?name=${user.Firstname}+${user.Lastname}&background=1a1f2e&color=fff&bold=true&size=128`} 
                    alt="provider" 
                    className="w-24 h-24 rounded-[2rem] border-4 border-white shadow-lg object-cover"
                />
                <div className="absolute -bottom-1 -right-1 w-6 h-6 bg-blue-500 border-4 border-white rounded-full shadow-sm"></div>
              </div>
              ...
              <div className="mb-6">
                ...
                <span className="inline-block mt-2 px-3 py-1 bg-blue-50 text-blue-600 rounded-lg text-[9px] font-black uppercase tracking-widest border border-blue-100">
                    {user.role}
                </span>
              </div>
              ...
              <div className="w-full grid grid-cols-2 gap-3 pt-6 border-t border-gray-50 mt-auto">
                ...
                >
                  <FiSettings strokeWidth={3} /> Manage
                </button>
                <button
